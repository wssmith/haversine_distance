...
public:
	...
	template<
		...
		(... && std::is_arithmetic_v<std::decay_t<Params>>)
		&& !std::is_base_of_v<std::discrete_distribution<>, std::decay_t<Distribution>>
		>
	>
	explicit random_generator(Params&&... args)
	{
		std::random_device dev;
		_impl = std::make_unique<random_generator_impl>(Engine{ dev() }, std::forward<Params>(args)...);
	}

	template<
		typename UnaryOperation,
		typename = std::enable_if<std::is_base_of_v<std::discrete_distribution<>, std::decay_t<Distribution>>>
	>
	random_generator(size_t nw, double x_min, double x_max, UnaryOperation fw)
	{
		std::random_device dev;
		_impl = std::make_unique<random_generator_impl>(Engine{ dev() }, nw, x_min, x_max, fw);
	}
