...
{
    const double dLat = radians_from_degrees(y1 - y0);
    const double dLon = radians_from_degrees(x1 - x0);
    const double lat1 = radians_from_degrees(y0);
    const double lat2 = radians_from_degrees(y1);

    const double a = square(std::sin(dLat / 2.0)) + std::cos(lat1) * std::cos(lat2) * square(std::sin(dLon / 2));
